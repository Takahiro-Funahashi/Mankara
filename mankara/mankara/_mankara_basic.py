...
class mankara_basic(BoardSurface):
    def __init__(self):
        super().__init__()

        # self._init_peaces_ = 8
        self.title = "Mankara Basic"
        self.isHelp = False
    ...
    def select_pocket(self, p, n):
        if self.player == p and n < POCKET_NUMBER-1:
            ...
            for _ in range(move_num):
                ...
                # 次のポケットの判定
                if number == POCKET_NUMBER-1:  # ゴールであるなら
                    # 次回は陣地変更
                    number = -1
                    turn = 1 - turn
    ...
    def judge(self):
        ...
        p0_num = sum(self.game_board[0][:POCKET_NUMBER-1])
        p1_num = sum(self.game_board[1][:POCKET_NUMBER-1])

        if self.player == 0 and p0_num == 0:
            msg = 'Player 0 Win!!'
        elif self.player == 1 and p1_num == 0:
            msg = 'Player 1 Win!!'
